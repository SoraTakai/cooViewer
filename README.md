...
- [`db20112`](https://github.com/SoraTakai/cooViewer/commit/db2011292b891b7352374274b30f2c923e39cb19): Add moved-file auto-apply preference for missing-path recovery. The initial basename remap has been replaced by signature-based identity (`historySignature`) using SHA-256 over sampled file bytes (`COHistorySignatureSampleBytes`, currently `1 MiB` from head and tail; full file for small files), and recovery state is canonicalized by signature so the latest close state wins across `RecentItems`, `LastPages`, and `BookSettings`. `RecentItems`/`LastPages` signature fallback also respects `AutoAcceptMissingSetting`.
